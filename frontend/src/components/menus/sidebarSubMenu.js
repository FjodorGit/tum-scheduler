...
const sidebarSubMenuOverlay = document.querySelector(
  ".sidebar-sub-menu__overlay",
);
...
// dark, light, contrast
const objectiveRadioBtns = document.querySelectorAll(".theme-radio__input");
const objectiveRadioOptions = ["dark", "light", "contrast"];
...
const shortcutSwitch = document.querySelector(
  ".smia-toggle-shortcuts-checkbox",
);
const animationsSwitchBtn = document.querySelector(".smdt-toggle-checkbox");
const notifyDisabledShortcutsIcon = document.querySelector(
  ".keyboard-disabled-sm",
);
...
export default function getSidebarSubMenu(store, context) {
  ...
  function addSelectedPrefix() {
    ...
    const prefixTag = createPrefixTag(prefix, context);
  ...
  function openSubMenu() {
    ...
    const themeIdx = objectiveRadioOptions.indexOf(context.getColorScheme());
    objectiveRadioBtns[themeIdx].checked = true;
  ...
  function setAnimationsIcons(val) {
    const parenticonwrapper = document.querySelector(
      ".toggle-animations-icon__sm",
    );
    const icons = {
